...
相比于Losketch佬和另一位大佬的[Unicode快闪生成器](https://github.com/tile-WWDCCS-name/unicode-flash-generator)，本代码更注重于简单粗暴轻量化，只需**一个**py文件和一些点缀的文件（图标、音乐）<del>以及几个前置库</del>组成，内有用Tkinter库制作的UI界面方便设置参数，前面两者使用的快闪生成器均采用图片/PP自动拼接的方式制作视频，而本代码使用的是<del>万能的</del>Pygame库直接在页面上以指定的帧数显示字符，你所需要的只是一个简单的60帧录屏软件（如OBS）设置好倒计时，等倒计时结束，快闪就会开始实时生成。在1.6版本，程序拥有自动生成视频的功能（需要手动勾选开启）。
<br/>

## 当前版本最低运行环境:- Python3.10<br/>
...
- [portable_ffmpeg](https://github.com/mrkbac/portable-ffmpeg)————用于一体化下载FFmpeg并执行相关代码；
  - **注意，需要连得上Github，首次运行py文件会从Github仓库下载ffmpeg到第三方库目录中以进行调用；**
...
* 请使用最新版本的pyinstaller打包，否则在打包numpy后会导致exe文件报错
...
3. easyfont库里的fonts
4. 音乐（这个就不用多说了吧）
5. portable_ffmpeg库里的binaries子目录文件夹（理论上可以不包，但是会每次生成视频都会下载一遍ffmpeg到缓存里，生成结束就销毁）
...
- 快捷拖拽字体/文本文件
- **自动化生成快闪视频并配上音乐**（需要手动勾选且成功安装了ffmpeg，且必须等实时快闪结束或中途关闭程序才可以生成）

## 接下来的计划

* [x]  增加快捷填写特定区块范畴的功能（可能又得借助某些库的力量）
